...
void rampStop(t_ramp *Ramp)
{
	long int absspeed_l;
	int ds,absspeed, absspeed_tmp;
	byte negative = 0;
	absspeed_l = Ramp->speed;
	if(absspeed_l < 0) {
		absspeed_l = -absspeed_l;
		negative = 1;
	}
	absspeed = absspeed_l >> (RAMP_VPOW + 1);
	absspeed_tmp = absspeed_l >> RAMP_UINCPOW;
	ds = (((long)(absspeed) * absspeed_tmp) / Ramp->maxDecel); //stop distance = speed²/2d
	if(negative) {
		Ramp->destPos = (Ramp->currentPos>>RAMP_UINCPOW) - ds;
		if(Ramp->destPos < 0) Ramp->destPos = 0;
	} else {
		Ramp->destPos = (Ramp->currentPos>>RAMP_UINCPOW) + ds;
	}
}
